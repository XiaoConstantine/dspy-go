...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --dataset-size)
            ...
            ;;
        -h|--help)
            echo "Usage: $0 [--optimizer bootstrap|mipro|simba|copro|gepa|all] [--dataset-size N]"
            echo "  --optimizer: Which optimizer to test (default: all)"
            echo "  --dataset-size: Dataset size for testing (default: 20)"
            echo "  Note: CoPro and GEPA may have different availability between Python and Go versions"
            exit 0
            ;;
        *)
