...
func TestTokenUsageFromContextIsolatesBranchesAndAggregatesParents(t *testing.T) {
	...
	require.EqualValues(t, 1, TokenUsageEventCount(leftCtx))
	require.EqualValues(t, 1, TokenUsageEventCount(rightCtx))
	require.EqualValues(t, 2, TokenUsageEventCount(rootCtx))
	require.EqualValues(t, 2, TokenUsageEventCount(ctx))
