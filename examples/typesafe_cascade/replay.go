...
func newCascadeClient(replay bool, model string) (decide.SystemOneClient, func(), error) {
	if !replay {
	...
	if err := json.Unmarshal(cascadeFixture, &recorded); err != nil {
		return nil, func() {}, fmt.Errorf("decode cascade replay fixture: %w", err)
	}
	if len(recorded) != len(cascadeTickets) {
		return nil, func() {}, fmt.Errorf("cascade replay fixture has %d records, want %d", len(recorded), len(cascadeTickets))
	}
	fixtures := make([]systemOneReplayFixture, len(recorded))
	for index, item := range recorded {
		if item.Ticket != cascadeTickets[index].Text {
			return nil, func() {}, fmt.Errorf("cascade replay fixture %d does not match the ticket", index)
		}
		fixtures[index] = systemOneReplayFixture{
			request:   expectedSystemOneReplayRequest(item.Ticket),
			response:  item.Response,
			requestID: fmt.Sprintf("req_cascade_replay_%d", index+1),
		}
	}
...
func newCascadeReplayLLM() (core.LLM, error) {
	return newTextReplayLLM("cascade-replay-llm", []recordedLLMResponse{
		{
			match:   cascadeTickets[1].Text,
			content: "category:\ntechnical",
			usage:   core.TokenInfo{PromptTokens: 112, CompletionTokens: 4, TotalTokens: 116},
		},
		{
			match:   cascadeTickets[3].Text,
			content: "category:\nproduct",
			usage:   core.TokenInfo{PromptTokens: 110, CompletionTokens: 4, TotalTokens: 114},
		},
		{
			match:   cascadeTickets[4].Text,
			content: "category:\naccount",
			usage:   core.TokenInfo{PromptTokens: 116, CompletionTokens: 4, TotalTokens: 120},
		},
	})
}
...
func newLiveLLM(apiKey string, model core.ModelID) (core.LLM, error) {
	...
	// This example measures provider calls and latency, so disable the factory's
	// transparent response cache rather than reporting cache hits as live calls.
